...
import allIconsMap from '../../utils/Icons/icons';
import synonyms from '../../utils/Icons/synonyms';


const allIcons = Object.keys(allIconsMap).map(key => allIconsMap[key])
      ...
      {icons.map(icon => {
        return (
          <span key={icon.key} className={clsx('markdown-body', classes.icon)}>
            <icon.Icon
              tabIndex={-1}
              onClick={handleClickOpen(icon, props.type)}
              title={icon.key}
...
Object.keys(allIconsMap).forEach(icon => {
  const { key } = allIconsMap[icon];
...
export default function SearchIcons(props) {
  ...
  return (
    <Dialog open={props.openDialog} onClose={props.onClose} fullWidth maxWidth="xl" fullScreen>
      ...
      <Grid container>
        ...
        <Grid item xs={12} sm={9}>
          ...
          <Icons type={props.type} icons={icons} classes={classes} handleClickOpen={props.handleClickOpen} />
        </Grid>
